...
def execute(context):
    ...
    df_population = context.stage("synthesis.population.sampled")[[
        ...
        "housing_type","household_type", "parking", "achlr"  # BIAO YIN 04/2023
    ]]
    ...
    df_population = pd.merge(df_population, df_hts_persons[[
        "hts_id", "hts_household_id", "has_license", "has_pt_subscription", "is_passenger", "parking_at_workplace", "commuting_dist_egt" # BYIN: 4-2023
    ]], on = "hts_id")

    df_population["parking_at_workplace"] = df_population.groupby("household_id")["parking_at_workplace"].transform("max")  # BYIN: 4-2023: parking_at_workplace (if at least 1) at the household level
    df_population["commuting_dist_egt"] = df_population.groupby("household_id")["commuting_dist_egt"].transform("max")
